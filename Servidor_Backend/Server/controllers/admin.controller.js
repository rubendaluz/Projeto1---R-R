...
export const registerAdmin = async (req, res) => {
  try {
    const { type, password, username, active } = req.body;
 ...
 const saltRounds = 10;
    ...
    const admin = await AdminModel.create({ type, password: hashedPassword, username,  active });
...
export const updateAdmin = async (req, res) => {
  try {
    const { id } = req.params;
    const { type, password, username,  active } = req.body;
    await AdminModel.update({ type, password, username,  active }, { where: { id } });
